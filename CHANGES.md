...
### 🐛 修复与测试优化 (Fix & Test Refactoring)

- **新增 V1 自动化测试**: 新建了一套针对 v1 版本 API 协议的单元测试套件 `tests/v1-api.test.ts`，涵盖路由翻译及客户端各种异常请求处理的深度校验。
- **防止 Mock 客户端报错**: 修复了执行模块命令过程中，测试 Mock 客户端无 `baseUrl` 字段引发 `TypeError` 崩溃的缺陷，采用可选链 `client.baseUrl?.endsWith` 安全读取。
- **Windows 路径兼容**: 修复了在 Windows 系统下进行构建打包时，测试硬编码斜杠 `/` 与平台路径 `\` 差异引起的冲突，改为使用 `node:path` 中的 `join`。
- **清除测试用例状态突变**: 修复了配置测试中直接突变修改全局共享 `mockProfile` 对象导致其携带 `config.defaultOutputFormat = 'json'` 配置污染，致使后续删除确认测试在并发运行环境下因格式被强转 json 而被跳过并报错的致命缺陷。
- **优化进程流交互时序**: 将批量删除交互测试 `stdin` 写入改为精准事件驱动机制（通过监测 `proc.stderr` 直至包含“确认删除”后才执行写入），移除了原有的不稳定等待，确保在并发多任务的 CI 环境下百分之百稳定跑通。
